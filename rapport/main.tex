...
\begin{figure}[h]
	\begin{tikzpicture}
		...
		\node[below = 0.5cm of intermediate,ellipse, draw](env) {Environnement};
		\node[below = of codegen,ellipse, draw](prims) {Primitives};
		
		\path[draw, ->] (env) -- (intermediate);
		\path[draw, ->] (env) -| ($(codegen.south) - (0.8cm, 0)$);
		...
		\node[above =0.1cm of outIR, ellipse, draw](symbTable) {Symboles};
		\path[draw, ->] (symbTable) -| (intermediate);
		\path[draw, ->] (symbTable) -| (codegen);
	\end{tikzpicture}
